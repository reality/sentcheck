...
sentiments.each { iri, v ->
  println "${++i}/${sentiments.size()} $iri"
	def ce = fac.getOWLClass(IRI.create(iri))

def label

  def ontology = doid
  if(v.o == 'MESH') { ontology = mesh }
EntitySearcher.getAnnotations(ce, ontology).each { anno ->
        def property = anno.getProperty()
        OWLAnnotationValue val = anno.getValue()
        if(val instanceof OWLLiteral) {
          def literal = val.getLiteral()
          if((property.isLabel() || property.toString() == "<http://www.w3.org/2004/02/skos/core#prefLabel>") && !label) {
						label = literal
          }
        }
  }

sentiments[iri]['label'] = label

println sentiments[iri]['label']
...
def out = [ ['iri', 'ontology', 'label', 'VeryPositive', 'Positive', 'Neutral', 'Negative', 'VeryNegative', 'count'].join('\t') ]
